...
par(mfrow=c(2,2))
plot(dt.subset$datetime, as.numeric(as.character(dt.subset$Global_active_power)), type ="l", col="black", ylab="Global Active Power", xlab="",  ylim=c(0, 7))

plot(dt.subset$datetime, as.numeric(as.character(dt.subset$Voltage)),type ="l",ylab = "Voltage", xlab = "datetime" ,col = "black")

plot(dt.subset$datetime, as.numeric(as.character(dt.subset$Sub_metering_1)),type ="l",ylab = "Energy Sub Metering", xlab = "" ,col = "black", ylim=c(0,40))
legend("topright", c("Sub_metering 1", "Sub_metering_2","Sub_metering_3"), lty= c(1,1), col = c("black", "red", "blue"), text.col = c("black", "red","blue"), cex=0.3, xjust=0.5)
...
plot(dt.subset$datetime, as.numeric(as.character(dt.subset$Global_reactive_power)), type ="l", col="black", ylim=c(0.0, 0.5), xlab = "datetime", ylab="Global_reactive_power")
